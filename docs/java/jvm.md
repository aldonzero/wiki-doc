...
* 在类编译过程中，会把**类元信息**放到**方法区**，类元信息的其中一部分便是类常量池，主要存放字面量和符号引用，而字面量的一部分便是文本字符
...
**逃逸分析**并不是直接的优化手段，而是一个**代码分析方式**，通过动态分析对象的作用域，为优化手段如栈上分配、标量替换和同步消除等提供依据，发生逃逸行为的情况有两种：**方法逃逸**和**线程逃逸**
